...
```bash
union run --remote llmops_rag/vector_store.py create --include_union --limit 10
```

Then run the simple rag pipeline

```bash
union run --remote llmops_rag/simple_rag.py run --question "Write a simple hello world workflow"
```
...
```bash
union run --remote llmops_rag/vector_store.py create_knowledge_base --include_union --limit 10
```

Then generate a question and answer dataset:

```bash
union run --remote llmops_rag/create_qa_dataset.py run --n_questions 1 --n_answers 5
```
...
```bash
union run --remote llmops_rag/hyperparam_opt.py run --hpo_configs config/prompt_experiment.yaml
```

Experiment with different chunksizes:

```bash
union run --remote llmops_rag/hyperparam_opt.py run --hpo_configs config/chunksize_experiment.yaml
```

Experiment with different splitters:

```bash
union run --remote llmops_rag/hyperparam_opt.py run --hpo_configs config/splitter_experiment.yaml
```

TODO:
- implement LLM as a judge to create annotated dataset
- implement random search
- implement bayesian hpo
