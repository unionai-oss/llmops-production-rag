...
```bash
union run --remote llmops_rag/synthesize_data.py run --n_questions 1 --n_answers 5
```
...
```bash
union run --remote llmops_rag/hpo.py run --hpo_configs config/prompt_experiment.yaml
```

Experiment with different chunksizes:

```bash
union run --remote llmops_rag/hpo.py run --hpo_configs config/chunksize_experiment.yaml
```

Experiment with different splitters:

```bash
union run --remote llmops_rag/hpo.py run --hpo_configs config/splitter_experiment.yaml
```
